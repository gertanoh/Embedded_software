...
void edge_detection()
{
    ...
    for(y = 0; y < 8; y++){
	    for(x = 8; x < 9; x++){
	        temp_array[x][y+1] = edge_right[x][y];
        }
    }
    /* add out_matrix */
    for(y = 1; y < 9; y++){
...
int main(void) {
   ...
   while(1){
        ...
        *flag_finish_sobel_3 = 0;
        *flag_finish_interpolation_3 = 0; 
        grayscale();
        interpolation();
        ...
        *flag_finish_interpolation_3 = 1;
        ...
        while(!( *flag_finish_interpolation_1 == 1 && *flag_finish_interpolation_2 == 1 && *flag_finish_interpolation_4 == 1 ));
