...
volatile int* const flag_inter_1 = (volatile int*) FLAG_FINISH_INTER_1;
volatile int* const flag_inter_2 = (volatile int*) FLAG_FINISH_INTER_2;
volatile int* const flag_inter_3 = (volatile int*) FLAG_FINISH_INTER_3;
volatile int* const flag_inter_4 = (volatile int*) FLAG_FINISH_INTER_4;
...
void shared2_chip_gray(unsigned char* input, unsigned char* output )
{
    ...
    x_size = 16;
    y_size = 16;
    x_offset = 0; // it is cpu_1, first quarter
    skip_offset = x_size ;
    
	
	// move pointer, we know it is cpu_1 so offset is fixed : 3
	input +=  3;
	for(y = 0; y < y_size ; y++){
	    for(x = 0; x < x_size  ; x++){
            temp = *input++;
...
int main(void) {
	...
	// wait for cpu_0 to initialise everyone
  //  delay(2);
    ...
    while(1){
		...
		* No need of flag because we have another sync point
		* down, so next time every cpu will have read his picture
		*/
        ...
        *flag_inter_1 = 1;
        ...
        while(!( *flag_inter_2 == 1 && *flag_inter_3 == 1 && *flag_inter_4 == 1 ));
