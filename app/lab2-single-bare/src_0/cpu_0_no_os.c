...
void resize()
{
   int x ,y; //Henry
    ...
    for(y = 0; y < size_y  ; y++)
        for( x = 0; x < size_x; x++){
            *resize_imaged++ = ( *gray + *(gray + 1) + *(gray + size_x)  + *(gray + size_x  + 1) ) >> 2 ;
            gray = gray + 2;
        }
}
void edge_detection()
{
    ...
    int* edge_value = (int*) ( pixel_pointer + ((size_x)) * ((size_y)) );    
    /* kernels */
    ...
    for( y = 0; y < size_y - 2; y++){
        for( x = 0; x < size_x - 2; x++){
            ...
            ( (*(pixel_pointer + size_x)) << 1) + ( (*(pixel_pointer + 2 + size_x)) << 1)+
            (-(*(pixel_pointer + (size_x <<1) ) ))  + ((*(pixel_pointer + 2 + (size_x<<1))));
            
            tmp_y = (-(*pixel_pointer)) + (-(*(pixel_pointer + 1)) <<1) + (-(*(pixel_pointer + 2)))+
            ((*(pixel_pointer + (size_x << 1)))) + ( (*(pixel_pointer + 1+ (size_x << 1))) << 1) +  *(pixel_pointer + 2 + (size_x<< 1) );
