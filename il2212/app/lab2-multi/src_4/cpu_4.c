...
void shared2_chip_gray(unsigned char* input, unsigned char* output )
{
    ...
    *input++;
    ...
    x_offset = ( (x_size << 1) * (y_size) * 3) + ( x_size  * 3 ) ;;//32*16*3 + 16*3; // it is cpu_4, fourth quarter, 
    skip_offset = x_size ;
...
void interpolation(unsigned char* in, unsigned char* out)
{
	...
	// calculate offset
	skip_offset = x_size;
	x_offset = ( (x_size << 1) * y_size) + x_size;
	out+= x_offset;
...
void edge_detection(unsigned char* in)
{
	...
	x_offset = ( (x_size - 1) <<1) * (y_size - 1) + (x_size - 1);//14*7 + 7;
	...
	in = in + ( ( x_size << 1) * (y_size - 1) ) + (x_size - 1);//16*7 + 7;
	// copy the new input
	unsigned int i, j ;
